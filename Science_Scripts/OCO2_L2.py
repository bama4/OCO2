...
#default root path
DEFAULT_PATH = "//home//bama4//OCO2_L2_Data"
EXT = ".h5"
...
def getRawCO2(data_file):
    ...
    S_M = data_file["RetrievalResults"]
    co2_readings = S_M["xco2"]
...
def getRawCO2Weak(data_file):
    ...
    S_R = data_file["SoundingMeasurements"]
    co2_readings = S_R["radiance_weak_co2"]
    return cos_readings

#obtain raw latitudes
def getRawLat(data_file):
    S_M = data_file["RetrievalGeometry"]
    LATS = data_file["retrieval_latitude"]
    return LATS

#obtain water indicator array
def getWaterArray(data_file):
    S_M = data_file["RetrievalGeometry"]
    WAT = S_M["land_water_indicator"]
    return WAT

#obtain raw longitudes
def getRawLong(data_file):
    S_M = data_file["RetrievalGeometry"]
    LONGS = S_M["retrieval_longitude"]
    return LONGS
...
#get .h5 files
   for f in files:
      if(f.name[-len(EXT):] == EXT):
         try:
             g = h5py.File(str(f.path),"r")
             f_h5.append(g)
         except:
             print("INVAILD " + str(f.path))
             continue
